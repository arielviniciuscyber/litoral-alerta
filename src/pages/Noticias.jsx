...
export default function NewsPage() {
  ...
  // Notícia destaque (primeira da lista)
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        {!carregando && !erro && destaque && currentPage === 1 && (
          <article className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 mb-10 hover:shadow-xl transition-all group cursor-pointer">
            <div className="flex flex-col lg:flex-row">
              <div className="lg:w-1/2 h-64 lg:h-auto overflow-hidden relative">
                <img
                  src={destaque.foto_capa}
                  alt="Destaque"
              ...
              <div className="lg:w-1/2 p-8 lg:p-10 flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-xs text-slate-400 font-medium">
                    {new Date(destaque.data_publicacao).toLocaleDateString("pt-BR", {
                ...
                <button className="font-display bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold px-6 py-3 rounded-xl shadow-sm shadow-blue-600/20 transition-all w-fit">
        ...
        {!carregando && !erro && (currentPage === 1 ? restantes : noticiasPaginadas).length > 0 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(currentPage === 1 ? restantes : noticiasPaginadas).map(noticia => (
              <article
                key={noticia.id}
                className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all group cursor-pointer flex flex-col"
              >
